...
public class FinanceRequirement extends Composite {
	...
	public FinanceRequirement(){}
	public FinanceRequirement(final ToolBarPanel toolbar) {
		...
		toolbar.setFinancialRequirement_Listener(new ToolBarPanel.FinancialRequirement_Listener() {
			...
			public void onDeleteButtonClick() {
				...
				final FinanceRequirements selectedfr = requestlist.getSelectedfr();
		...
		requestlist.setListener(new RequestList.Listener() {
			public void onTableSelected(FinanceRequirements requestselected) {
				docklayoutpanel.clear();
		...
		requestdetail.setListener(new RequestDetail.Listener() {
			@Override
			public void onApproveClick(final FinanceRequirements item, final String comment) {
				final Date date = new Date(System.currentTimeMillis());
			...
			@Override
			public void onDenyClick(final FinanceRequirements item, final String comment) {
				final Date date = new Date(System.currentTimeMillis());
				if(comment.equals("")==false){
					userservices.getUserName(new AsyncCallback<String>() {
						public void onFailure(Throwable caught) {}
			...
			@Override
			public void onMoreInfoClick(final FinanceRequirements item, final String comment) {
				final Date date = new Date(System.currentTimeMillis());
				if(comment.equals("")==false){
					userservices.getUserName(new AsyncCallback<String>() {
						public void onFailure(Throwable caught) {}
